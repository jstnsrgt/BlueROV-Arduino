...
private:
    ...
    void Construct(const char* _host, const int _port, const char* _tenantId, const char* _user, const char* _password, const char* _applicationKey, Connection4G* conn, IoTShield* shield);
...
public:
    ...
    /**
      ...
      *     @param applicationKey - for authorization
      */
    TelstraIoT(const char* host, const char* tenantId, const char* user, const char* password, const char* applicationKey, Connection4G* conn, IoTShield* shield);
    virtual ~TelstraIoT();
	...
	/**
	  * Write Telstra IoT credentials to secure non-volatile memory on IoT Shield
	  */
	void writeCredentials(const char *deviceID,const char *deviceTenant,const char *deviceUsername,const char *devicePassword);

	/**
	  * Read Telstra IoT credentials from secure non-volatile memory on IoT Shield
	  */
	void readCredentials(char *deviceID,char *deviceTenant,char *deviceUsername,char *devicePassword);

	/**
	  * Clear Telstra IoT credentials from secure non-volatile memory on IoT Shield
	  */
